Spree::Variant.class_eval do
  ...
  accepts_nested_attributes_for :volume_prices, allow_destroy: true,
    ...
    def price_in(currency)
       if sale_price.present?
         ...
         Spree::Price.new(:variant_id => self.id, :amount => self.price, :currency => currency)
       end
  ...
  def compute_volume_price_quantities(type, default_price, quantity, user)
    if sale_price.present?
      default_price = sale_price
    end
    puts default_price.inspect
   
    volume_prices = join_volume_prices nil
